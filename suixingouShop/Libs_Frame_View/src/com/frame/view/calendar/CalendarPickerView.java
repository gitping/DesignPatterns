...
package com.frame.view.calendar;
...
import com.frame.view.calendar.MonthCellDescriptor.RangeState;
...
public class CalendarPickerView extends ListView {
  public enum SelectionMode {
    ...
    /**
     * 随心购自定义单选模式
     * 和之前单选模式类似，只有在选择已选项的时候，是变成取消选中状态的
     */
    SXGSINGLE
  }
  ...
  private boolean doSelectDate(Date date, MonthCellDescriptor cell) {
    ...
    switch (selectionMode) {
      case SXGSINGLE:
    	  date = sxgSelect(date, newlySelectedCal);
    	  break;
      case RANGE:
  ...
  private Date sxgSelect(Date date, Calendar selectedCal){
	  for (MonthCellDescriptor selectedCell : selectedCells) {
		  selectedCell.setSelected(false);
	      if (selectedCell.getDate().equals(date)) {        
	        selectedCells.remove(selectedCell);
  ...
  private class DefaultOnInvalidDateSelectedListener implements OnInvalidDateSelectedListener {
    ...
    //选择超出初始化时间段的时间时，不需要提示了，需要就加上这段文字
      /*String errMessage =
