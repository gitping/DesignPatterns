...
final class CameraConfigurationManager {
	...
	void setDesiredCameraParameters(Camera camera) {
		...
		// modify here
		if(BaseCaptureActivity.orientationType != 1){
			if (hasGingerbread()) {
	...
	private static Point findBestPreviewSizeValue(
		...
		Log.w(TAG, previewSizeValueString + "\n" + screenResolution);
		// float bi = screenResolution.y / (float) screenResolution.x;
		int sub = Math.abs(screenResolution.y - screenResolution.x);
		int bestX = 0;
		int bestY = 0;
		// float newBi = 0;
		int newSub = 0;
		...
		for (String previewSize : COMMA_PATTERN.split(previewSizeValueString)) {
			...
			// newBi = newY / (float) newX;
			newSub = Math.abs(newY - newX);
			if (Math.abs(sub - newSub) < diff) {
				diff = Math.abs(sub - newSub);
				bestX = newX;
				bestY = newY;
			}
		}
		Point res;
		if (bestX > 0 && bestY > 0) {
			res = new Point(bestY, bestX);
		} else {
			res = new Point(screenResolution.y, screenResolution.x);
		}
