...
public abstract class Cshop_CategoryAdapter extends BaseAdapter {
	Context context;
     ...
     * @param adapter
     */
    public void updateAdapter(String title,Adapter adapter)
    {
    	for (int i = 0; i < categories.size(); i++) {
			if (categories.get(i).getTitle().equals(title)) {
				categories.add(i, new Cshop_Category(title, adapter));
			}
		}
		notifyDataSetChanged();
    }
