...
//				FMakeRequest.parseParameter(t, RedEnvelopesStatisticsResJo.class, requestCallBack);
	...
	public void getCollectRewardDetailData(CollectBillDetailReqJo indreq,final FRequestCallBack requestCallBack){
		FrameRequest fr=FMakeRequest.getCollectRe(indreq);
		FinalHttp http=AfinalUtil.getFinalHttp();
		http.post(fr.getUrl(), fr.getAp(), new AjaxCallBack<String>() {
			@Override
			public void onSuccess(String t) {
				super.onSuccess(t);
				Trace.i("RedEnvelopeDealMessageActivityHelper--getCollectRewardDetailData: onSuccess " + t);
				Gson gs=new Gson();
				ResponseDTO2<CollectBillDetailResJo, Object> dto2 = gs.fromJson(t, new TypeToken<ResponseDTO2<CollectBillDetailResJo, Object>>() {}.getType());
				requestCallBack.onSuccess(dto2);
			}
			...
			public void onFailure(Throwable t, int errorNo, String strMsg) {
				super.onFailure(t, errorNo, strMsg);
				requestCallBack.onFailure(t, errorNo, strMsg);
				Trace.i("RedEnvelopeDealMessageActivityHelper--getCollectRewardDetailData: onFailure " + t);
			}
